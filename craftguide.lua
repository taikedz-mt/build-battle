...
local function colorize(str)
	return str --minetest.colorize("#FFFF00", str)
end
...
function craftguide:get_formspec(player_name, no_recipe_update)
	...
	minetest.show_formspec(player_name, "build_battle:book", formspec)
end
...
function craftguide:get_items(player_name)
	...
	for name, def in pairs(minetest.registered_nodes) do
		if not (def.groups.not_in_creative_inventory == 1) and
			...
			def.name:find("build_battle:") and
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname ~= "build_battle:book" then return end
...
minetest.register_craftitem("build_battle:book", {
	...
	on_use = function(itemstack, user)
		...
		if not datas[player_name] then
			...
			minetest.show_formspec(player_name, "build_battle:book",
